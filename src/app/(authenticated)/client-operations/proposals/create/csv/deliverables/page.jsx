          ...
          <div className="mt-8 rounded-2xl bg-white p-8 shadow">
            <div className="mb-6">
              ...
              <p className="text-gray-600">
                Review the data before importing. This will create or update {validationData.totalRows} deliverable template(s).
              </p>
            </div>

            {/* Phase → Deliverables Count */}
            {validationData.phaseDeliverableCounts && (
              <div className="mb-6 rounded-lg border border-blue-200 bg-blue-50 p-4">
                <h3 className="text-sm font-semibold text-gray-900 mb-3">Deliverables by Phase</h3>
                <div className="space-y-2">
                  {Object.entries(validationData.phaseDeliverableCounts).map(([phase, count]) => (
                    <div key={phase} className="flex items-center justify-between text-sm">
                      <span className="text-gray-700">{phase}</span>
                      <span className="font-medium text-blue-600">{count} deliverable(s)</span>
                    </div>
                  ))}
                </div>
              </div>
                ...
                {validationData.headers.map((header, idx) => (
                  <div key={idx} className="flex items-center justify-between text-sm">
                    ...
                    <span className="font-medium text-green-600">
                      {header === 'Deliverable Name' ? 'deliverableLabel' : 
                       header === 'Phase Name' ? 'phase (reference)' : 
                       header === 'Description' ? 'description' : 
                       header === 'Quantity' ? 'quantity (reference)' : 
                       header === 'Unit' ? 'defaultUnitOfMeasure' : 
                       header === 'Duration (Days)' ? 'defaultDuration' : 
                       'mapped'}
                    </span>
                  </div>
                ))}
